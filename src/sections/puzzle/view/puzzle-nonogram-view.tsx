...
export function PuzzleNonogramView() {
  ...
  const currentPreset = useMemo(
    () => NONOGRAM_PRESETS.find((p) => p.id === selectedPresetId) || NONOGRAM_PRESETS[0],
    [selectedPresetId]
  );
  ...
  const handleSelectPreset = useCallback(
    (presetId: string) => {
      ...
      const preset = NONOGRAM_PRESETS.find((p) => p.id === presetId) || NONOGRAM_PRESETS[0];
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              ...
              <Box sx={{ display: 'flex' }}>
                ...
                <Box sx={{ display: 'flex' }}>
                  {currentPreset.colClues.map((clues, c) => (
                    <Box
                      key={c}
                      sx={{
              ...
              {grid.map((row, r) => {
                const targetRowClues = currentPreset.rowClues[r];
                const rowDone = isRowSatisfied(row, targetRowClues);
                ...
                return (
                  <Box key={r} sx={{ display: 'flex' }}>
                    ...
                    {row.map((cellState, c) => {
                      const isHinted = hintCellCoord === `${r}-${c}`;
          ...
          >
            💡 팁: PC에서는 마우스 <b>우클릭</b>으로 바로 X 표시를 남길 수 있습니다. 5칸마다 굵은
            선으로 구분됩니다.
          </Typography>
        </Card>

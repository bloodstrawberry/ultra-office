...
export function ImageToolView() {
  ...
  const renderAnonCanvas = (src: string, intensity: number, mode: 'pixelate' | 'blur') => {
    ...
    img.onload = () => {
      ...
      const faceY = (img.height - faceH) / 3;
      ...
      if (mode === 'pixelate') {
        ...
        for (let y = 0; y < faceH; y += blockSize) {
          for (let x = 0; x < faceW; x += blockSize) {
            const pixelIndex = (y * faceW + x) * 4;
            const r = data[pixelIndex];
            const g = data[pixelIndex + 1];
            const b = data[pixelIndex + 2];
            const a = data[pixelIndex + 3];

            for (let dy = 0; dy < blockSize && y + dy < faceH; dy += 1) {
              for (let dx = 0; dx < blockSize && x + dx < faceW; dx += 1) {
                const targetIdx = ((y + dy) * faceW + (x + dx)) * 4;
                data[targetIdx] = r;
                data[targetIdx + 1] = g;
                data[targetIdx + 2] = b;
                data[targetIdx + 3] = a;
    ...
    <DashboardContent>
      ...
      >
        ...
        <Tab
          ...
          iconPosition="start"
        />
        <Tab label="4. GIF 애니메이션" value="gif" icon={<GifRoundedIcon />} iconPosition="start" />
        <Tab
          ...
          >
            ...
            >
              ...
              <input
                ...
                accept="image/*"
                onChange={(e) => {
                  if (e.target.files) setGifFiles(Array.from(e.target.files));
                }}
              />
            </Button>

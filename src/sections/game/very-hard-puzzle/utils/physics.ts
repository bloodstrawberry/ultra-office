...
export const tryMoveBlock = (
  ...
  const canMoveThisBlock = getBlockProperties(block, currentGrid)?.canSelect || isMovingWall;
  ...
  if ((block === BLOCK_WALL_H || block === BLOCK_AUTO_WALL_H) && dy !== 0) return fail;
  if ((block === BLOCK_WALL_V || block === BLOCK_AUTO_WALL_V) && dx !== 0) return fail;
  // Standard match blocks can only slide horizontally
  if (!isMovingWall && dy !== 0) return fail;
  ...
  if (isMovingWall) {
    ...
    while (ky >= 0) {
      ...
      const baseAbove = getBaseBlockId(aboveBlock);
      const canMoveOnWall =
        aboveBlock !== BLOCK_EMPTY &&
        (getBlockProperties(aboveBlock, currentGrid)?.canSelect ||
          getBlockProperties(aboveBlock, currentGrid)?.canFall ||
          isStrawBlock(aboveBlock) ||
          baseAbove === BLOCK_WALL_H ||
          baseAbove === BLOCK_WALL_V ||
          baseAbove === BLOCK_AUTO_WALL_H ||
          baseAbove === BLOCK_AUTO_WALL_V);
      if (!canMoveOnWall) {
        break;
      }
      coords.push({ x, y: ky });

...
import type { GameContentProps } from './game-view-types';
...
import { EditorPalette } from './editor/editor-palette';
import { HintViewModal } from './modals/hint-view-modal';
import { ToastNotification } from './toast-notification';
import { UndoGuideModal } from './modals/undo-guide-modal';
import { RecordViewModal } from './modals/record-view-modal';
import { playEngineSound as playSound } from '../utils/sound';
import { formatLevelsJSON } from '../utils/format-levels-json';
import { CheaterDetectModal } from './modals/cheater-detect-modal';
import { ALL_PAINT_TOOLS, useEditorHotkeys } from '../utils/hot-key';
import { JsonExportImportModal } from './modals/json-export-import-modal';
import { copyGrid, BUILTIN_LEVELS, findInitialCursor } from '../utils/types';
import { getItem, setItem, getWatchedHintStages } from '../utils/local-storage';
...
export function GameContent({
  ...
  const handleStartFromRecord = (snapshotGrid: CellType[][], recordIndex: number) => {
    setPlayTestMode(true);
    const newGrid = copyGrid(snapshotGrid);
    setGrid(newGrid);
    const initCursor = findInitialCursor(newGrid);
    setCursor(initCursor);
    setGrabbed(false);
    setIsCursorVisible(false);
    setIsGameOver(false);
    ...
    const updated = recordedSteps.slice(0, recordIndex + 1);
    recordedStepsRef.current = updated;
    setRecordedSteps(updated);

    toast.openToast('선택한 화면에서 테스트를 시작합니다!');
    playSound('start', muted);
  };

  const handleStartFromHint = (snapshotGrid: CellType[][]) => {
    setPlayTestMode(true);
    const newGrid = copyGrid(snapshotGrid);
    setGrid(newGrid);
    const initCursor = findInitialCursor(newGrid);
    ...
    const initialSnapshot = copyGrid(newGrid);
    recordedStepsRef.current = [initialSnapshot];
    setRecordedSteps([initialSnapshot]);

    toast.openToast('선택한 힌트 화면에서 테스트를 시작합니다!');
    playSound('start', muted);
  };
  ...
  return (
    <div className="flex h-full min-h-0 items-center justify-center p-0 md:pt-0 md:pb-1 md:px-4 text-stone-800 overflow-hidden relative font-sans select-none w-full">
      ...
      <HintViewModal
        ...
        muted={muted}
        onToast={(msg) => toast.openToast(msg)}
        onStartFromHint={handleStartFromHint}
      />
      ...
      <RecordViewModal
        ...
        onStartFromRecord={handleStartFromRecord}
      />

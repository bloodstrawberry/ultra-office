const STORAGE_PREFIX = 'ultra-office:delivery:';
const storageKey = (key: string) => `${STORAGE_PREFIX}${key}`;

function clearDeliveryStorage() {
...
const Storage = {
  getItem: async (key: string) => localStorage.getItem(storageKey(key)),
  setItem: async (key: string, value: string) => localStorage.setItem(storageKey(key), value),
  removeItem: async (key: string) => localStorage.removeItem(storageKey(key)),

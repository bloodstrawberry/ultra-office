...
export default function StagePreviewModal({
  ...
  useEffect(() => {
    if (!isOpen || !previewContainerRef.current) return undefined;
    const updateSize = () => {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-center z-10 w-full shrink-0 pt-0.5">
          <div className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-amber-100/90 border border-amber-300/80 rounded-full text-amber-950 font-black text-sm sm:text-base shadow-xs">
            <span>🧩</span>
            <span>{stageNum}번 문제</span>
          </div>
        </div>
        ...
        <div className="w-full flex-1 flex items-center justify-between gap-2 sm:gap-3 z-10 min-h-0 my-2">
          ...
          <button
            ...
            className={`w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center transition-all border-2 shrink-0 ${
              hasPrev
                ? 'bg-gradient-to-b from-amber-300 via-amber-400 to-amber-500 border-amber-600/80 text-amber-950 shadow-md shadow-amber-500/30 hover:scale-110 active:scale-90 hover:brightness-110 cursor-pointer'
                : 'bg-amber-100/20 border-amber-200/40 text-amber-900/20 opacity-30 cursor-not-allowed shadow-none'
            }`}
            aria-label="이전 문제"
            title={hasPrev ? '이전 문제 보기' : '첫 번째 문제입니다'}
          >
          ...
          <div
            ...
            className="flex-1 h-full max-h-[290px] sm:max-h-[330px] flex items-center justify-center p-2 sm:p-3 bg-amber-950/10 rounded-2xl border-2 border-amber-400/50 shadow-inner overflow-hidden min-w-0"
          >
            <div
            ...
            >
              {/* SVG Filter for Organic Soil Edges */}
              <svg className="absolute" width="0" height="0" aria-hidden="true">
                <defs>
                  <filter id="previewFarmEdge" x="-4%" y="-4%" width="108%" height="108%">
                    <feTurbulence
                      type="fractalNoise"
                      baseFrequency="0.04"
                      numOctaves="4"
              ...
              {/* Soil Background Layer */}
              <div
                className="relative w-full h-full flex justify-center"
                style={{ filter: 'url(#previewFarmEdge)' }}
              >
                <div
          ...
          <button
            ...
            aria-label="다음 문제"
            title={hasNext ? '다음 문제 보기' : '다음 문제가 잠겨있습니다'}
          >
        ...
        <div className="flex items-center gap-3 w-full pt-1.5 z-10 shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-3 px-4 bg-gradient-to-b from-amber-100 to-amber-200 hover:from-amber-200 hover:to-amber-300 active:scale-95 text-amber-950 font-black text-sm sm:text-base rounded-2xl border-2 border-amber-400/80 shadow-md transition-all cursor-pointer"
          >
            취소
          </button>
          <button
            type="button"
            onClick={onStart}
            className="flex-1 py-3 px-4 bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-600 hover:from-emerald-400 hover:to-teal-400 active:scale-95 text-white font-black text-sm sm:text-base rounded-2xl border-2 border-emerald-400/80 shadow-xl shadow-emerald-600/40 transition-all cursor-pointer flex items-center justify-center gap-2 hover:scale-[1.02]"
          >
            <span>🚀</span>
            <span>풀어볼게!</span>
          </button>

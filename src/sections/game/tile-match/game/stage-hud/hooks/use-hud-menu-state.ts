...
export interface UseHudMenuStateOptions {
...
export function useHudMenuState({
  ...
  useEffect(() => {
    ...
    if (!isEditor) {
      getItem('ultra-office:tile-match:puzznic_max_unlocked').then((stored) => {
        if (stored) {
          setStageInputValue(stored);
        }
      });
    }
  ...
  const handleGoHome = () => {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('tile-match-go-home'));
    }
  };

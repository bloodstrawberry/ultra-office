...
const PADDING_SAMPLE_IMAGES: SampleImageItem[] = [
  {
    id: 'sample-product',
    label: '단색 배경 제품 컷 (여백 확장 & 스마트 그라데이션)',
    url: 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=800&auto=format&fit=crop&q=80',
    subLabel: '제품 썸네일',
  },
  {
    id: 'sample-portrait',
    label: '인물 프로필 사진 (블러 배경 & 비율 맞춤)',
    url: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=800&auto=format&fit=crop&q=80',
    subLabel: '프로필 증명사진',
  },
  {
    id: 'sample-graphic',
    label: '그래픽 일러스트 (단색 / 투명 여백 확장)',
...
export function PhotoPaddingView() {
  ...
  const [isSpoidMode, setIsSpoidMode] = useState<boolean>(false);
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (pixel[3] === 0) {
      ...
      renderPaddingPreview({ bgType: 'color', solidColor: hex });
      setIsSpoidMode(false);
      toast.success(`색상이 추출되어 배경색으로 지정되었습니다: ${hex}`);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await downloadDataUrl(dataUrl, `padded_photo_${Date.now()}.png`);
    ...
    >
      <Box sx={{ mb: { xs: 1.5, sm: 2 }, flexShrink: 0 }}>
        <Typography variant="h4" sx={{ fontWeight: 800, mb: 0.5 }}>
          여백 조정 (Padding Studio)
        </Typography>
        <Typography variant="body2" sx={{ color: 'text.secondary' }}>
          사진의 상하좌우 여백을 확장하고 스마트 그라데이션, 블러, 테두리 평균 색상 등 최적의 배경을
          채웁니다.
        </Typography>
      </Box>
      ...
      {!imageSrc ? (
        <PhotoUploadWorkspace
          sampleImages={PADDING_SAMPLE_IMAGES}
          onSelectSample={loadSampleImage}
          onFileSelect={processFile}
          title="이미지 업로드"
          subtitle="여백을 확장하거나 인스타 피드, 스토리, 배너 비율에 맞출 사진을 선택하세요."
          icon={<AspectRatioRoundedIcon sx={{ fontSize: 36 }} />}
        />
      ) : (
        <Box
          sx={{
            ...
            >
              ...
              >
                <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>
                  편집 캔버스 {isSpoidMode && '(스포이드 모드: 사진을 클릭하여 색상 추출)'}
                </Typography>
              ...
              >
                <canvas
                  ...
                  style={{
                    ...
                    cursor: isSpoidMode ? 'crosshair' : 'default',
                    maxWidth: '100%',
                    maxHeight: '100%',
                    objectFit: 'contain',
              ...
              {paddingBgType === 'color' && (
                <Box sx={{ mb: 2, p: 1.5, bgcolor: 'action.hover', borderRadius: 2 }}>
                  ...
                  >
                    ...
                    <Button
                      size="small"
                      variant={isSpoidMode ? 'contained' : 'outlined'}
                      color="primary"
                      onClick={() => {
                        setIsSpoidMode((prev) => !prev);
                        if (!isSpoidMode) {
                          toast.info('사진에서 추출할 배경색 지점을 클릭하세요.');
                        }
                      }}
...
export { PhotoPaddingView as PaddingView };

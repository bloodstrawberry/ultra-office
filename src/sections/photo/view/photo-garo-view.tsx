...
export function GaroView() {
  const [files, setFiles] = useState<UploadedFile[]>([]);
  const [cropSettings, setCropSettings] = useState<CropSettings>(DEFAULT_HORIZONTAL_SETTINGS);
  ...
  useEffect(() => {
    getHorizontalCropSettings().then((saved) => {
  ...
  const updateCropSettings = (newSettings: CropSettings) => {
    ...
    if (isLoaded) {
      saveHorizontalCropSettings(newSettings);
    }
  };
  ...
  const handleSingleDownload = async (file: UploadedFile) => {
    ...
    const filename = `garo_${cropSettings.width}x${cropSettings.height}_${file.name}.png`;
  ...
  const handleBatchDownload = async () => {
    ...
    try {
      const entries: ZipFileEntry[] = readyItems.map((f, idx) => ({
        filename: `garo_${cropSettings.width}x${cropSettings.height}_${idx + 1}_${f.name}.png`,
        data: f.resultUrl!,
      }));
  ...
  return (
    ...
    >
      <AppsInTossNavHeader currentTab="garo" />
            ...
            <Card sx={{ p: 2.5, borderRadius: 3 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
                <SafeNumberInput
                  ...
                  fallbackValue={DEFAULT_HORIZONTAL_SETTINGS.width}
                ...
                <SafeNumberInput
                  ...
                  fallbackValue={DEFAULT_HORIZONTAL_SETTINGS.height}

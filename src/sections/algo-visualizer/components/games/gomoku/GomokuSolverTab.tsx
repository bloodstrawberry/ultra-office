...
export function GomokuSolverTab() {
  ...
  const [currentProblem, setCurrentProblem] = useState<GomokuProblem>(GOMOKU_PUZZLE_LIST[0]);
  const [isCatalogOpen, setIsCatalogOpen] = useState(false);

  const [board, setBoard] = useState<GomokuGrid>(() => createEmptyGomokuBoard());
  const [history, setHistory] = useState<GomokuGrid[]>([]);
  const [lastMove, setLastMove] = useState<GomokuPoint | null>(null);
  const [winLine, setWinLine] = useState<GomokuWinLine | null>(null);
  const [turn, setTurn] = useState<GomokuColor>('B');
  const [isAIMoving, setIsAIMoving] = useState<boolean>(false);

  const [currentNodeTree, setCurrentNodeTree] = useState<GomokuSolutionNode[]>([]);
  const [statusMessage, setStatusMessage] = useState<string>('');
  const [isSolved, setIsSolved] = useState<boolean>(false);
  const [isFailed, setIsFailed] = useState<boolean>(false);
  const [showHint, setShowHint] = useState<boolean>(false);
  const [showSolution, setShowSolution] = useState<boolean>(false);
  const [aiAnalysis, setAiAnalysis] = useState<GomokuAIAnalysis | null>(null);

  // ================= Sandbox Mode States =================
  const [sandboxBoard, setSandboxBoard] = useState<GomokuGrid>(() => createEmptyGomokuBoard());
  const [sandboxHistory, setSandboxHistory] = useState<GomokuGrid[]>([]);
  const [sandboxLastMove, setSandboxLastMove] = useState<GomokuPoint | null>(null);
  const [sandboxWinLine, setSandboxWinLine] = useState<GomokuWinLine | null>(null);
  const [sandboxTurn, setSandboxTurn] = useState<GomokuColor>('B');
  const [placementTool, setPlacementTool] = useState<PlacementTool>('play');
  const [sandboxAnalysis, setSandboxAnalysis] = useState<GomokuAIAnalysis | null>(null);

  const setupProblem = useCallback((problem: GomokuProblem) => {
    const newBoard = createEmptyGomokuBoard();
    ...
    const analysis = analyzeGomokuPosition(newBoard, problem.playerColor);
    setAiAnalysis(analysis);
  }, []);

  useEffect(() => {
    setHasLoaded(true);
    setupProblem(GOMOKU_PUZZLE_LIST[0]);
    const initEmpty = createEmptyGomokuBoard();
    setSandboxBoard(initEmpty);
    setSandboxHistory([initEmpty]);
  }, [setupProblem]);

  const handleSelectProblemIndex = (index: number) => {
    if (index >= 0 && index < GOMOKU_PUZZLE_LIST.length) {
      setSelectedProblemIndex(index);
      const prob = GOMOKU_PUZZLE_LIST[index];
  ...
  const handleNextProblem = () => {
    if (selectedProblemIndex < GOMOKU_PUZZLE_LIST.length - 1) {
      handleSelectProblemIndex(selectedProblemIndex + 1);
    }
  };

  const handleRandomProblem = () => {
    const randomIndex = Math.floor(Math.random() * GOMOKU_PUZZLE_LIST.length);
  ...
  const handleUndo = () => {
    ...
    const analysis = analyzeGomokuPosition(targetBoard, 'B');
    setAiAnalysis(analysis);
  };

  const handlePlaceStone = (p: GomokuPoint) => {
    if (isSolved || isFailed || isAIMoving || turn !== 'B' || board[p.r][p.c] !== null) return;

    playBadukStoneSound();
    const nextBoard = board.map((row, r) =>
      row.map((cell, c) => (r === p.r && c === p.c ? ('B' as GomokuColor) : cell))
    );
    ...
    // Check Win
    const win = checkGomokuWin(nextBoard, p);
    if (win) {
      setWinLine(win);
      setIsSolved(true);
      playPuzzleSolvedSound();
      setStatusMessage('5목 완성! 흑의 완벽한 승리입니다!');
      return;
    }

    const matchingNode = findMatchingGomokuNode(currentNodeTree, p);
    ...
    if (matchingNode) {
      ...
      if (matchingNode.aiResponse) {
        ...
        setTimeout(() => {
          ...
          playBadukStoneSound();
          const aiBoard = nextBoard.map((row, r) =>
            row.map((cell, c) => (r === aiMove.r && c === aiMove.c ? ('W' as GomokuColor) : cell))
          );
          setBoard(aiBoard);
          ...
          const aiWin = checkGomokuWin(aiBoard, aiMove);
          if (aiWin) {
            setWinLine(aiWin);
            setIsFailed(true);
            playPuzzleFailedSound();
            setStatusMessage('백이 5목을 완성했습니다. 실패!');
            return;
          }
          ...
          if (matchingNode.children && matchingNode.children.length > 0) {
            ...
            setIsSolved(true);
            playPuzzleSolvedSound();
            setStatusMessage('축하합니다! 오목 전술 퍼즐을 완벽하게 풀어냈습니다.');
          }
        }, 500);
      } else {
        setIsSolved(true);
        playPuzzleSolvedSound();
        setStatusMessage(matchingNode.comment || '정답입니다! 승리 결정타를 놓았습니다.');
      }
    } else {
      // Dynamic AI move
      setIsAIMoving(true);
      setStatusMessage(`흑 ${formatGomokuCoord(p)} 착수. 백(AI)이 수읽기 중입니다...`);

      setTimeout(() => {
        const { move: aiMove, reason } = findBestGomokuAIMove(nextBoard, 'W');
        ...
        if (aiMove) {
          ...
          if (aiWin) {
            ...
            setIsFailed(true);
            playPuzzleFailedSound();
            setStatusMessage('백이 5목을 완성했습니다!');
          } else {
            setStatusMessage(
              `백이 ${formatGomokuCoord(aiMove)}에 응수했습니다 (${reason}). 흑의 다음 수를 두세요!`
            );
          }
        }
    ...
    const analysis = analyzeGomokuPosition(nextBoard, 'B');
    setAiAnalysis(analysis);
  };
  ...
  const handleAutoPlaySolution = () => {
    ...
    if (currentNodeTree.length === 0 || isSolved || isFailed || turn !== 'B') {
      ...
      setTimeout(() => {
        ...
        if (firstNode) {
          handlePlaceStone(firstNode.move);
        }
      }, 150);
      return;
    }
    ...
    if (targetNode) {
      handlePlaceStone(targetNode.move);
    }
  };

  const getGomokuSolutionSteps = (problem: GomokuProblem) => {
    const steps: string[] = [];
    const traverse = (nodes: GomokuSolutionNode[], stepNum: number) => {
      if (!nodes || nodes.length === 0) return;
      const node = nodes[0];
      steps.push(`${stepNum}수: 흑(Black) ${formatGomokuCoord(node.move)} 착수`);
      if (node.aiResponse) {
        steps.push(`${stepNum + 1}수: 백(White) ${formatGomokuCoord(node.aiResponse)} 응수`);
        if (node.children) {
  ...
  const handleSandboxUndo = () => {
    ...
    setSandboxAnalysis(analyzeGomokuPosition(target, sandboxTurn === 'B' ? 'W' : 'B'));
  };

  const handleSandboxClickPoint = (p: GomokuPoint) => {
    if (placementTool === 'play') {
      if (sandboxBoard[p.r][p.c] !== null) return;

      playBadukStoneSound();
      const nextBoard = sandboxBoard.map((row, r) =>
        row.map((cell, c) => (r === p.r && c === p.c ? sandboxTurn : cell))
      );

      const win = checkGomokuWin(nextBoard, p);
      if (win) {
        setSandboxWinLine(win);
        playPuzzleSolvedSound();
      }

      const nextTurn = sandboxTurn === 'B' ? 'W' : 'B';
      setSandboxBoard(nextBoard);
      setSandboxHistory((prev) => [...prev, nextBoard]);
      setSandboxLastMove(p);
      setSandboxTurn(nextTurn);
      setSandboxAnalysis(analyzeGomokuPosition(nextBoard, nextTurn));
      ...
      const next = sandboxBoard.map((row, r) =>
        row.map((cell, c) => (r === p.r && c === p.c ? ('W' as GomokuColor) : cell))
      );
      playBadukStoneSound();
      setSandboxBoard(next);
      setSandboxHistory((prev) => [...prev, next]);
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, pb: 4 }}>
        ...
        <>
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
              ...
              >
                <AutoAwesomeRoundedIcon sx={{ color: '#d97706' }} />
              ...
              <Button
                ...
                sx={{
                  ...
                  '&:hover': { background: 'rgba(217, 119, 6, 0.12)' },
                }}
              >
                퍼즐 목록 ({selectedProblemIndex + 1} / {GOMOKU_PUZZLE_LIST.length})
              </Button>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
              ...
              <IconButton
                ...
                disabled={selectedProblemIndex === GOMOKU_PUZZLE_LIST.length - 1}
                sx={{ color: '#64748b' }}
                title="다음 문제"
              >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                  {isSolved ? (
                    ...
                    <PlayArrowRoundedIcon sx={{ color: '#d97706', fontSize: 28 }} />
                  )}
                  <Typography
                    variant="h6"
                    sx={{
              ...
              {showSolution && (
                ...
                >
                  ...
                  >
                    {getGomokuSolutionSteps(currentProblem).map((s, idx) => (
                      <Typography
                        key={idx}
                        variant="body2"
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  <HelpOutlineRoundedIcon sx={{ color: '#d97706', fontSize: 20 }} />
              ...
              {aiAnalysis && (
                <GameAlgorithmInspector
                  ...
                  algorithmName="패턴 인식(Pattern Scoring) & Minimax 수읽기"
                  complexityInfo={{
                    time: 'O(b^d) → 국소 후보점 제한 탐색',
                    space: 'O(N^2) 그리드 배열',
                    branchingFactor: 'b ≈ 8~25 (인접 활성 영역)',
                  }}
                />
              )}
            </Box>
          ...
          >
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center' }}>
              <GomokuBoard
                ...
                winLine={sandboxWinLine}
                onPlaceStone={handleSandboxClickPoint}
              />
            </Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Card sx={{ p: 2.5, bgcolor: '#f8fafc', border: '1px solid #e2e8f0' }}>
                ...
                {sandboxWinLine && (
                  ...
                  >
                    <EmojiEventsRoundedIcon sx={{ color: '#16a34a', fontSize: 32 }} />
                    <Box>
                      <Typography variant="subtitle1" sx={{ fontWeight: 900, color: '#15803d' }}>
                        🎉 {sandboxWinLine.color === 'B' ? '흑 (Black)' : '백 (White)'} 5목 승리!
                      </Typography>
                      <Typography variant="body2" sx={{ color: '#166534' }}>
                        5개의 돌이 완벽하게 연결되어 승리했습니다.
                      </Typography>
                    </Box>
                  </Box>
                )}
                ...
                <Typography variant="body2" sx={{ color: '#64748b', lineHeight: 1.6 }}>
                  ...
                  <br />• <strong>대국 모드:</strong> 흑과 백이 번갈아 15x15 판 위를 클릭하여 착수할
                  수 있으며, 5목이 완성되면 즉시 승리 하이라이트가 표시됩니다.
                  <br />• <strong>돌 배치 모드:</strong> 흑/백 돌을 원하는 곳에 자유롭게 놓아 4-3,
                  3-3 공격 수순을 연습해보세요.
                </Typography>
              </Card>
            </Box>
      ...
      >
        ...
        <DialogContent sx={{ p: 2.5, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          {GOMOKU_PUZZLE_LIST.map((prob, idx) => {
            ...
            return (
              <Card
                ...
                sx={{
                  p: 2,
                  cursor: 'pointer',
                  bgcolor: isCurrent ? 'rgba(217, 119, 6, 0.08)' : '#ffffff',
                  border: '1px solid',
                  borderColor: isCurrent ? '#d97706' : '#e2e8f0',
                  '&:hover': {
